...
### 🎨 **Beautiful Welcome Page** (Just like FastAPI!)

SyntroJS automatically creates a modern landing page at the root route:
...
// You'll see a beautiful landing page with links to documentation! 🎉
```
...
```javascript
// Development: Full documentation (default)
const dev = new SyntroJS({ title: 'API' });
// ✅ Landing page ✅ /docs ✅ /redoc ✅ /openapi.json

// Production: Disable docs for security
const prod = new SyntroJS({ title: 'API', docs: false });
// ❌ No documentation exposed

// Granular control per environment
const custom = new SyntroJS({
  title: 'API',
  docs: {
    landingPage: true,
    swagger: false,  // Don't expose interactive UI in prod
    redoc: false,
    openapi: true    // Keep spec for tooling
  }
});
```

🔒 **Security Tip:** Never expose `/docs` or `/redoc` in production - they're like a "hacking manual" exposing your API endpoints!

📖 **Full documentation:** See [Documentation Configuration Guide](./docs/DOCUMENTATION_CONFIG.md)
...
**For production deployments, ALWAYS disable documentation:**
...
- [ ] **Enable rate limiting** for PP endpoints
- [ ] **Configure proper logging** without sensitive data
- [ ] **Use HTTPS** in production
- [ ] **Set environment variables** for secrets

### 📦 CDN vs Local Assets (Optional Dependencies)

**Default Behavior (99% of users):**
- Documentation assets are loaded from CDN (jsdelivr.net)
- No additional installation required
- Works out of the box with internet access

**For Air-Gapped Environments (Optional):**

If you need documentation in air-gapped, banking, or government environments:
...
**Auto-Detection:** SyntroJS automatically detects if optional dependencies are installed and serves assets locally. No configuration needed!
...
Currently, the landing page HTML is embedded. Future enhancement will allow custom HTML:

```javascript
// Future API
const app = new SyntroJS({
  title: 'My API',
  customLandingPage: './static/index.html'
});
```
...
- **Welcome page** - Modern landing page at `/` (just like FastAPI!)
- **Interactive docs** - Swagger UI + ReDoc out of the box
- **Production-ready config** - Disable docs with `docs: false` for security
