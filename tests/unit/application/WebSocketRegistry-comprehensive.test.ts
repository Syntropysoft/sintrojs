...
import type { WebSocketHandler, RequestContext } from '../../../src/domain/types';
...
describe('WebSocketRegistry - Comprehensive Coverage Tests', () => {
  ...
  describe('Guard Clauses', () => {
    it('should validate path parameter', () => {
      ...
      // Test null path
      expect(() => registry.add(null as any, handler)).toThrow('Path is required and must be a valid string');
      
      // Test undefined path
      expect(() => registry.add(undefined as any, handler)).toThrow('Path is required and must be a valid string');
      
      // Test empty path
      expect(() => registry.add('', handler)).toThrow('Path is required and must be a valid string');
      
      // Test non-string path
      expect(() => registry.add(123 as any, handler)).toThrow('Path is required and must be a valid string');
    });

    it('should validate handler parameter', () => {
      // Test null handler
      expect(() => registry.add('/ws', null as any)).toThrow('WebSocket handler must be a valid function');
      
      // Test undefined handler
      expect(() => registry.add('/ws', undefined as any)).toThrow('WebSocket handler must be a valid function');
      
      // Test non-function handler
      expect(() => registry.add('/ws', 'invalid' as any)).toThrow('WebSocket handler must be a valid function');
      expect(() => registry.add('/ws', 123 as any)).toThrow('WebSocket handler must be a valid function');
      expect(() => registry.add('/ws', {} as any)).toThrow('WebSocket handler must be a valid function');
    });
  });
